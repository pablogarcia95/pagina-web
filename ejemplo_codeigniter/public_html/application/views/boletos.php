	...
	<div id="heading">
		...
		<?php
			if($tor == -1 || $tor != $par['torneo_id']) {
				...
				echo "<div class='tbl' onselectstart='return false'>
					<table><tr><td colspan='7'>Torneo: ".$par["torneo_nombre"]."</td></tr>
					<tr>
						<td>Ref</td>
						<td>Hora</td>
						<td>Fecha</td>
...
echo "<tr id='info'><td>".$par['par_id']."</td><td>".$par['par_hora']."</td><td>".$par['par_fecha']."</td><td>".$par['par_equipo1'].'-Vs-'.$par['par_equipo2']."</td><td id='q2'>".$par['par_porcentaje1']."</td><td id='t1'>".$par['par_porcentaje3']."</td><td id ='y8'>".$par['par_porcentaje2']."</td></tr>";
		...
		<div id="calcula" name="calcula">
			...
			<label style="font-weight:bold; font-size:14px;"> Monto de la apuesta en </label>
			<select  id="mon_id" name="mon_id">
			...
			<input value="5000" type="number" id="base" name="base"/>
...
<!-- Scripts -->
	...
	<script>
				...
				$.post("agregar_ticket_item",
			        ...
			        function(data,status){
			        	if (data == '-1'){
			        		...
			        		alert('El ticket se realizó con éxito, imprima su tiquete');
			        		//alert(data);
			        		var datos=data.split('::');
						Popup($(elem).prop('outerHTML'), datos[0], datos[1], datos[2], datos[3]);
			        	}
					
			        });
		        }
		        else{
		        	alert('Deberá seleccionar como mínimo una apuesta ');
		        }
		}
	
		function imprime(elem){
			var r = confirm("Desea generar un Ticket para ésta apuesta?");
			if (r == true) {
				getTabla(elem);
			} else {
		...
		function Popup(da, ticket, fecha, hora, moneda) 
		    {
		        var mywindow = window.open('', 'my div', 'height=400,width=600');
		        mywindow.document.write('Nota: Expira a los cinco (5) dias');
		        mywindow.document.write('<br>http://win7-24.com/');
		        mywindow.document.write('<br>');
		        mywindow.document.write('<br>Ticket Nro. '+ticket);
		        mywindow.document.write('<br>Fecha - Hora: '+fecha+' - '+hora);
		        mywindow.document.write('<br>Jugada en '+moneda + ': '+addCommas($('#base').val()));
		        mywindow.document.write('<br>----------------------------------------');
		        mywindow.document.write('<br>'+da);
		        mywindow.document.write('<br>----------------------------------------');
		        mywindow.document.write('<br>Premio Pesos: ' + $('#monto').val());
		        mywindow.document.write('<br>----------------------------------------');
		        mywindow.document.write('<br>La casa se reserva el derecho a suspender<br> cualquier jugada donde tenga algún error humano');
		        mywindow.print();
		        mywindow.close();
		        return true;
		    }
		...
		function val_rep(element, col){
			var c1 = rgb2hex(element.find("td").eq(4).css("background-color"));
			var c2 = rgb2hex(element.find("td").eq(5).css("background-color"));
			var c3 = rgb2hex(element.find("td").eq(6).css("background-color"));
			//alert(col);
			var c1_=false;
			var c2_=false;
			var c3_=false;
		...
		$("tr#info td").click(function() {     // function_td
			...
			if (col_index >= 4 && col_index <=6){
				if ( rgb2hex($(this).css("background-color"))=='#000000'){ 
					if (val_rep($(this).parent(), col_index)){
						var partido = rgb2hex($(this).parent().find("td").eq(3).text());
